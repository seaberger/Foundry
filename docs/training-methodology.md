...
| Run | Rank | LR | Train Loss | Eval Score | Regression |
|---|---|---|---|---|---|
| SFT v1 | 16 | 2e-5 | 1.52 | 2.0/10 | -6.8 from ORPO v2 |
| SFT v2 | 8 | 1e-6 | 1.68 | 2.2/10 | -6.7 from ORPO v2 |

Even at lr=1e-6 (100x lower) with rank 8 (minimal LoRA capacity), SFT destroys the ORPO character signal completely. The model reverts to base assistant behavior: bullet points, modern academic prose, third-person references to Madison, contractions.

**Approach used:** Merged ORPO v2 adapter into base Qwen 3-32B → applied fresh SFT LoRA on merged model → trained on 510 introspection examples (generated by Gemma 3 v4 model, filtered for voice quality) → 1 epoch.

Scripts: `experiments/qwen3-val/modal_train_sft_qwen.py`
W&B: `sbergman/foundry/runs/d4ciuqdl` (v1), check project for v2

**Root Cause Analysis — OPEN QUESTIONS:**

The failure is structural, not hyperparameter-related. Several hypotheses need investigation:

1. **Cross-model data contamination.** The SFT data was generated by Gemma 3 v4, not Qwen 3. Despite passing voice quality filters, the data may carry implicit Gemma-style patterns (tokenization artifacts, sentence structure preferences) that pull Qwen 3 away from its ORPO-trained character distribution.

2. **ORPO already includes SFT.** ORPO's objective is `SFT_loss + λ * preference_loss`. The SFT component within ORPO may already be doing what a subsequent SFT stage would do, and stacking a second SFT on top creates catastrophic interference with the preference-learned character signal. The Maiya et al. paper used DPO (pure preference, no SFT component), not ORPO — their two-stage pipeline (DPO then SFT) may not translate to (ORPO then SFT).

3. **Model scale effects.** Maiya et al. used smaller models (7-13B). At 32B parameters, the model's pretrained distribution is much stronger. A small SFT dataset (510 examples) may be insufficient to nudge the model toward introspection without also pulling it back toward pretrained assistant behavior — the introspection data is a subset of the behavior space the model needs to maintain.
...
**Actual spend to date:** ~$53 Modal (Gemma training/eval + Qwen validation + Qwen ORPO v1/v2 + rejected gen + merge + SFT x2 + eval x4) + ~$12 Sonnet (teacher + judge x6) = **~$65 total**
Out-of-pocket remaining: ~$450+ in Modal credits.
