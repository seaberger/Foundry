"""Full ORPO training for Madison character on Qwen 3-32B.

Uses the complete v4 dataset (1,273 pairs) with production hyperparameters
to get a direct comparison against Gemma 3 ORPO v4 (8.17/10).

Usage:
    modal run modal_train_orpo_qwen_full.py
    modal run modal_train_orpo_qwen_full.py --lr 1e-5 --output-name madison-qwen3-v2
    modal run modal_train_orpo_qwen_full.py --list-models
    modal run modal_train_orpo_qwen_full.py --get-adapter madison-qwen3-v1
"""
...
app = modal.App("foundry-qwen3-full")
...
def train_qwen3_orpo(
    # Data — full v4 dataset (already on Modal volume from Gemma training)
    dpo_data_path: str = "/adapters/data/madison-dpo.jsonl",
    # Model
    base_model: str = "Qwen/Qwen3-32B",
    max_seq_length: int = 2048,
    # LoRA config — rank 64 (GGUF robust)
    lora_rank: int = 64,
    lora_alpha: int = 64,
    lora_dropout: float = 0.0,
    # ORPO hyperparameters — lr=2e-5 matches Gemma v4 production
    beta: float = 0.1,
    ...
    # Output
    output_name: str = "madison-qwen3-v1",
    # Resume
    ...
    """Run full QLoRA ORPO training on Qwen 3-32B."""
    import json
    import os
    from pathlib import Path

    print(f"{'='*60}")
    print(f"Foundry — Qwen 3-32B Full ORPO Training")
    print(f"{'='*60}")
...
def main(
    ...
    epochs: int = 3,
    output_name: str = "madison-qwen3-v1",
    dataset: str = "../../data/training/madison-orpo-v4.jsonl",
    upload_data_flag: bool = True,
    list_models: bool = False,
    get_adapter: str = "",
    resume_from: str = "",
):
    """Run full Qwen 3-32B ORPO training on Modal.
    ...
    if upload_data_flag and data_path.exists():
        print(f"Uploading {data_path} ({data_path.stat().st_size / 1024 / 1024:.1f} MB)...")
    ...
    print(f"\nLaunching full Qwen 3-32B ORPO training on {GPU}...")
