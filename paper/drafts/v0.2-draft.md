...
### 3.7 Introspection SFT (Stage 2) — Abandoned
...
Switching to Qwen 3-32B with the same training data produced **8.81/10 corrected**, demonstrating that base model architecture choice meaningfully impacts character imprinting quality. Subsequent Round 2 training with source-enriched data (1,498 pairs) achieved our best result: **8.97/10 corrected** (Section 5.13). Qwen 3's pure ForCausalLM architecture eliminated all Gemma 3 infrastructure issues while improving character_consistency by +2.25 points and private_voice by +1.70 points.
